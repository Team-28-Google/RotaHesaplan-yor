...
- [x] ~~(V2)~~ Konumdan otomatik şehir tespiti — TAMAM (13 Tem, 4.0c Geocoding ile; tüm-Türkiye).
...
- [x] (Polish, 13 Tem) Biniş/iniş duraklarına hat renginde durak noktası + timeline yarı-katlanır
  (kapalıyken rozet özeti + saat + ok; dokununca alternatifler+adımlar; yeni bacakta kapanır).
...
- [x] **Geocoding API** ★ — TAMAM (13 Tem): /detect-city + CityPicker "Konumumdan bul";
  üstüne TÜM-TÜRKİYE geldi — 81 il norm_city'de, bilinmeyen ilde AI il merkezinden üretir
  (Antalya canlı testi ✓). Telefonda çalışması push bekliyor.
- [x] **Roads API** — TAMAM (13 Tem): /snap-track; yolculuk bitişinde iz yola oturtulur
  (servis hazır değilse ham iz). Canlı test: 3 nokta → 20 snap noktası ✓.
